...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  if (!isOpen) {
    return (
      <div className="hidden lg:flex w-12 sidebar-container bg-white dark:bg-dark-surface border dark:border-dark-border rounded-xl flex-col items-center py-4 gap-6 h-full transition-all shrink-0">
  ...
  return (
    <div className="w-full lg:w-80 sidebar-container flex flex-col bg-white dark:bg-dark-surface border dark:border-dark-border rounded-xl shadow-lg lg:shadow-sm h-full overflow-hidden transition-all shrink-0 z-50">
      <div className="p-4 border-b dark:border-dark-border flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/20">
        ...
        <button onClick={onToggle} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg text-slate-500"><ChevronLeft size={18} /></button>
      </div>
        ...
        {activeTab === 'templates' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setEditingProfile({ id: Math.random().toString(36).substr(2, 9), name: '', categoryId: categories[0]?.id || '', defaultDuration: 60, color: 'blue', resourceIds: [] })} className="p-1 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded"><Plus size={16} /></button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2">
              {profiles.map((p, idx) => (
                ...
                >
                  ...
                  <div className="absolute right-2 lg:right-8 top-1/2 -translate-y-1/2 flex gap-1 opacity-100 lg:opacity-0 group-hover:opacity-100 bg-white/90 dark:bg-slate-900/90 p-1 rounded shadow-sm border dark:border-slate-700">
        ...
        {activeTab === 'categories' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setEditingCategory({ id: Math.random().toString(36).substr(2, 9), name: '', color: 'blue' })} className="p-1 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded"><Plus size={16} /></button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2">
        ...
        {activeTab === 'resources' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setEditingResource({ id: Math.random().toString(36).substr(2, 9), name: '', description: '' })} className="p-1 hover:bg-amber-50 dark:hover:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded"><Plus size={16} /></button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2">
              {resources.map((r, idx) => (
                <div 
                ...
                >
                  ...
                  <p className="text-[9px] text-slate-400 dark:text-slate-500">{r.description || 'No notes'}</p>
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div className="space-y-3 mt-4">
                     <div>
                       ...
                       <div className="flex items-center gap-3">
                         ...
                         <span className="text-[10px] whitespace-nowrap text-slate-400">{formatTime(lunchRule.startTime)}</span>
                       </div>
                     </div>
                     <div>
                       ...
                       <div className="flex items-center gap-3">
                         ...
                         <span className="text-[10px] whitespace-nowrap text-slate-400">{formatTime(lunchRule.endTime)}</span>
                       </div>
                     </div>
                  </div>
                )}
             ...
             <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                   <button onClick={onExportCFP} className="flex items-center justify-center gap-2 p-3 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-lg text-xs font-bold border border-indigo-100 dark:border-indigo-900/50 hover:bg-indigo-100 transition-colors">
                     <Download size={14} /> <span className="hidden sm:inline">Export</span> .CFP
                   </button>
                   <button onClick={() => fileInputRef.current?.click()} className="flex items-center justify-center gap-2 p-3 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-700 hover:bg-slate-100 transition-colors">
                     <Upload size={14} /> <span className="hidden sm:inline">Import</span> .CFP
                   </button>
                   <input 
                     type="file" 
                     ref={fileInputRef} 
      ...
      {editingProfile && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-dark-surface rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4 border dark:border-slate-700">
             ...
             }} className="w-full py-3 text-sm font-bold text-white bg-indigo-600 rounded-xl">Apply Changes</button>
          </div>
        </div>
      )}
