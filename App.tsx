...
import { Clock, Wand2, RefreshCw, Layers, Moon, Sun, Monitor, Smartphone, Type, ZoomIn, ZoomOut, Library, LayoutGrid, Settings2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [profiles, setProfiles] = useState<ProfileBlock[]>(INITIAL_PROFILES);
  const [categories, setCategories] = useState<Category[]>(INITIAL_CATEGORIES);
  const [resources, setResources] = useState<Resource[]>(INITIAL_RESOURCES);
  const [selectedBlockId, setSelectedBlockId] = useState<string | null>(null);
  const [lunchRule, setLunchRule] = useState<LunchBreakRule>({ enabled: true, startTime: 720, endTime: 780 });
  ...
  const handleAddBlockFromProfile = (profile: ProfileBlock) => {
    ...
    // On mobile, automatically switch to timeline to see the new block
  ...
  const handleImportCFP = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        alert("Failed to load .CFP file. Format might be invalid.");
      }
    };
  ...
  const handleExportCSV = () => {
    const csv = generateCSV(blocks, categories, resources);
  ...
  const optimizeWithAI = async () => {
    ...
    try {
      ...
      const prompt = `Analyze this industrial schedule for equipment bottlenecks. 
      Equipment: ${resources.map(r => r.name).join(', ')}. 
      Blocks: ${blocks.map(b => `${b.title} uses ${b.resourceIds.map(rid => resources.find(r => r.id === rid)?.name).join(', ')}`).join('\n')}`;
  ...
  return (
    <div className="h-screen bg-slate-50 dark:bg-dark-bg text-slate-900 dark:text-slate-100 flex flex-col overflow-hidden transition-colors duration-300">
      <nav className="h-14 lg:h-16 bg-white dark:bg-dark-surface border-b dark:border-dark-border px-4 lg:px-6 flex items-center justify-between sticky top-0 z-50 shrink-0 no-print">
        <div className="flex items-center gap-2 lg:gap-3">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-[8px] lg:text-[10px] text-indigo-500 dark:text-indigo-400 font-bold uppercase tracking-widest leading-none mt-0.5">Industrial Planner</p>
          </div>
        </div>

        <div className="flex items-center gap-2 lg:gap-6">
           <div className="flex items-center gap-1 lg:gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
              <button onClick={() => adjustZoom(-0.2)} className="p-1 lg:p-1.5 rounded-md hover:bg-white dark:hover:bg-slate-700 text-slate-500 transition-all"><ZoomOut size={14} /></button>
              ...
              <button onClick={() => adjustZoom(0.2)} className="p-1 lg:p-1.5 rounded-md hover:bg-white dark:hover:bg-slate-700 text-slate-500 transition-all"><ZoomIn size={14} /></button>
           </div>

           <div className="flex items-center gap-1 lg:gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
              <button 
                onClick={() => setOrientation('portrait')} 
                className={`p-1 lg:p-1.5 rounded-md transition-all ${orientation === 'portrait' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`}
              >
                <Smartphone size={14} />
              </button>
              <button 
                onClick={() => setOrientation('landscape')} 
                className={`p-1 lg:p-1.5 rounded-md transition-all ${orientation === 'landscape' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`}
              >
                <Monitor size={14} />
              </button>
           </div>
           ...
           <div className="flex items-center gap-2 border-l dark:border-slate-800 pl-3 lg:pl-6">
              <button 
                ...
                className="p-1.5 lg:p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 dark:text-slate-400 transition-colors"
              >
      ...
      <main className="flex-1 p-2 lg:p-6 flex gap-4 lg:gap-6 overflow-hidden relative">
        ...
        `}>
          <Sidebar 
            ...
            lunchRule={lunchRule} 
            onUpdateLunchRule={setLunchRule}
            isOpen={isLeftPanelOpen || mobileTab === 'sidebar'}
            onToggle={() => {
              if (window.innerWidth < 1024) setMobileTab('timeline');
              else setIsLeftPanelOpen(!isLeftPanelOpen);
            }}
            onExportCFP={handleExportCFP}
        ...
        <div className={`flex-1 h-full ${mobileTab === 'timeline' ? 'block' : 'hidden lg:block'}`}>
          <Timeline 
            ...
            onSelectBlock={(id) => {
              ...
              if (window.innerWidth < 1024 && id) setMobileTab('detail');
            }} 
        ...
        `}>
          <DetailPanel 
            ...
            onToggle={() => {
              if (window.innerWidth < 1024) setMobileTab('timeline');
              else setIsRightPanelOpen(!isRightPanelOpen);
            }}
          />
        </div>
      </main>
      ...
      <div className="lg:hidden h-16 bg-white dark:bg-dark-surface border-t dark:border-dark-border flex items-center justify-around px-2 z-[70] no-print">
         <button 
           ...
           className={`flex flex-col items-center gap-1 flex-1 transition-colors ${mobileTab === 'sidebar' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'}`}
         >
           <Library size={20} />
         ...
         <button 
           ...
           className={`flex flex-col items-center gap-1 flex-1 transition-colors ${mobileTab === 'timeline' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'}`}
         >
           <LayoutGrid size={20} />
         ...
         <button 
           ...
           className={`flex flex-col items-center gap-1 flex-1 transition-colors ${mobileTab === 'detail' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'}`}
         >
           <Settings2 size={20} />
      ...
      <footer className="hidden lg:flex h-10 bg-white dark:bg-dark-surface border-t dark:border-dark-border px-6 items-center justify-between text-[10px] text-slate-400 dark:text-slate-500 font-medium shrink-0 no-print">
        ...
        <div className="flex gap-4">
          <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-indigo-500"></div> System Status: ONLINE</span>
        </div>
      </footer>
    </div>
  );
